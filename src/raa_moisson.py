...
def inventorier(dept, depuis, sources):
    ...
    annees = [(u, t) for u, t in _liens(doc_racine, r"/RAA/\d{4}$", base)
              if int(re.search(r"(\d{4})$", u).group(1)) >= depuis]
    ...
    for url_an, lib_an in annees:
        ...
        print(f"  {an} : {len(mois)} mois")
        for url_mois, lib_mois in mois:
            if url_mois in deja:
                continue
...
def etat(dept):
    ...
    print(f"  inventaire : {len(fics)} fichier(s), "
          f"{len({f['page_recueil'] for f in fics})} recueil(s)")
    if fics:
        resumer_inventaire(inv)
...
def main():
    ...
    ap.add_argument("--depuis", type=int, default=2016)
    ...
    if a.moissonner:
        moissonner(a.dept, garder_pdf=a.garder_pdf, plafond=a.plafond)
    if a.etat:
        etat(a.dept)
    if a.termine:
        total, faits, erreurs = etat(a.dept)
