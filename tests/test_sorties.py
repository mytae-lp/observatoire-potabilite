...
def _aplati(texte):
    """Une forme comparable : entités résolues, espaces réduits, sans casse.

    Le texte visible d'une page garde ses entités (`d&#x27;y`) là où la charge
    utile porte l'apostrophe. Comparer les deux sans les ramener à la même
    forme ne trouverait jamais rien — et le contrôle se croirait passant.
    """
    return re.sub(r"\s+", " ", html.unescape(texte or "")).strip().lower()


def _textes_cites(bloc):
    """
    Ce que la page CITE, et qui n'est donc pas la prose du projet.

    LE DÉFAUT QUE CECI CORRIGE, ET IL A BLOQUÉ UNE PUBLICATION.
    Le 17 août 2026, la reconstruction s'est arrêtée sur 1 233 prescriptions
    et 45 comparaisons anonymes. Aucune n'était de nous : toutes venaient de
    la **conclusion de l'ARS**, que la fiche affiche entre guillemets dans sa
    double lecture — « Je vous invite à prendre toutes les mesures nécessaires
    à la recherche de la cause de la pollution… ». C'est l'ARS qui prescrit, à
    un exploitant, dans son propre texte ; nous ne faisons que le montrer, et
    le montrer est ce que le §8bis n° 6 demande.

    Le contrôle avait raison de voir ces phrases, et tort de nous les
    attribuer. Il ne lisait la page que depuis le 16 août — avant, la fiche
    était fabriquée par `fiche.js` et la page était vide, si bien que la
    citation échappait à l'examen au lieu d'en être exclue.

    **La liste des termes interdits n'est pas touchée**, et elle ne doit pas
    l'être : c'est le périmètre de l'examen qui était faux, pas son exigence.

    La citation est reconnue par la charge utile et non par le balisage :
    `official.concl` porte la conclusion telle que la source l'a écrite. Un
    conteneur de mise en forme se renomme au premier remaniement de la fiche ;
    cette clé-là dit ce que la valeur EST.
    """
    cites = []
    for d in (bloc or {}).values():
        if not isinstance(d, dict):
            continue
        concl = (d.get("official") or {}).get("concl")
        if concl and concl.strip() not in ("", "—"):
            cites.append(_aplati(concl))
    return cites
